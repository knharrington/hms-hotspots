...
# filtered_sf <- st_as_sf(filtered, coords = c("UNIQUE_RET_LON", "UNIQUE_RET_LAT"), crs = st_crs(gridshp))
# 
# # Perform the spatial join
# grid_join <- setDT(st_join(filtered_sf, gridshp, join = st_intersects))
# 
# # Filter and aggregate the data
# filtered_grid <- grid_join %>%
#     group_by(GRID_ID) %>%
#     dplyr::reframe(
...
# gridvalues <- st_as_sf(merge(x = gridshp, y = filtered_grid, by = "GRID_ID", all.x = FALSE))
# 
# goodgrid <- gridvalues %>% filter(CLASSIFICATION == "Good")
# badgrid <- gridvalues %>% filter(CLASSIFICATION == "Bad")
# centroids <- gridvalues %>%
#       st_centroid()%>%
#       mutate(INTENSITY = scales::rescale(NUM_BLUEFIN, to = c(0, 1)))
# 
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
#                 popup = paste0(badgrid$GRID_ID)) %>%
#     addPolygons(data = goodgrid,
#                 fillColor = "#036BA1",
#                 weight = 3,
#                 opacity = 1,
#                 fillOpacity = 0.25,
...
# ex_points <- noaa_data %>%
#   filter(SPECIES_NAME %in% c("SWORDFISH"))
#   
# leaflet() %>% addTiles() %>%
#   addMarkers(data=ex_points, lat=~CENTROID_LAT, lng=~CENTROID_LON)
